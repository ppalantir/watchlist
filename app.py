...
app.config['SECRET_KEY'] = 'dev' # flash函数传递信息的密钥。# 等同于 app.secret_key = 'dev'
...
def login():
	if request.method == 'POST':
		...
		flash('Invalid useranme or password.') # 如果验证失败，显示错误信息
...
class User(db.Model, UserMixin): # 保存用户信息，表名将会是 user（自动生成，小写处理）
...
def edit(movie_id):
	...
	if request.method == 'POST': # 处理编辑表单的提交请求
		title = request.form['title'] # 要更新成的数据
